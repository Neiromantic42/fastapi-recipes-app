...
# Базовая схема книги рецептов, содержит общие поля, чтобы избежать дублирования
class BaseCookBook(BaseModel):
    ...
    cooking_time: int = Field(
        ...
        le=960 # Максимально время приготовления блюда 16 часов (например холодец)
    )

    ingredients: str = Field(
        ..., # Ингридиенты блюда, обязательный параметр
        title='Ингридиенты блюда (картошка, морковь, лук...)', # Название поля в документации
...
class CookBookOut(BaseCookBook):
    ...
    id: int = Field(
        title='Уникальный идентификатор рецепта'
    )
    ...
    class Config:
        orm_mode = True # Включаем поддержку работы с ORM-объектами (например, SQLAlchemy)

# Схема, которая будет возвращаться пользователю отсортированным списком объектов с прописанными миже полями
class CookBookShort(BaseModel):
    dish_name: str = Field(title='Имя блюда целиком') # Название поля в документации
    # Добавляем поле счетчика просмотров
    views_counter: int  = Field(title='Счетчик просмотров конкретного рецепта') # Название поля в документации
    # Добавляем поле время приготовления в минутах
    cooking_time: int = Field(title='Время приготовления блюда') # Название поля в документац